...
# Clone project
git clone https://github.com/GLlei/nideshop-admin.git
...
```
>默认账户 admin 密码 123456 为本地测试，对接接口后请使用接口账户 以下注释部分为接口调用
```
    ...
    Login({ commit }, userInfo) {   
      ...
      return new Promise((resolve, reject) => {
        if ( username == "admin" && userInfo.password == 123456 ) {
          setToken("sdasj2dsaj2as")
          commit('SET_TOKEN', "sdasj2dsaj2as")
          resolve()
        } else {
          reject("账号或密码错误")
        }
